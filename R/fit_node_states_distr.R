...
fit_node_states_distr <- function(bn, node, op = "proba", distr = 'beta', method = "mme",
  ...
  node <- node$posterior
  if (missing(discrete)) {
    if (is.element(distr, c("binom", "nbinom", "geom",
                               "hyper", "pois"))){
      discrete <- TRUE
    } else {
      discrete <- FALSE
  ...
  tmp <- lapply (as.list(1:ncol(node)), function(i) {
    fitdist(node[, i], distr = distr[i] , method = method[i],
            start = start[i], fix.arg = fix.arg[i], discrete = discrete[i], keepdata = keepdata[i], keepdata.nb = keepdata.nb[i], ...)
  })
